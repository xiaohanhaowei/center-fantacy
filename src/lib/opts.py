...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--debugger_theme', default='white', 
                             choices=['white', 'black'])
    self.parser.add_argument('--vis_graph', action='store_true',
                             help='visualise model architecture')
    ...
    self.parser.add_argument('--hr_conf', type=str, default=None, 
                             help='the whole architecture designation, when hrnet is defined, hr_conf must be defined')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':  # what task is this string respond for? 
    ...
    elif opt.task == 'ctdet' or opt.task == 'hrdet':
    ...
    if opt.arch == 'hrnet':
        assert opt.hr_conf is not None, 'hrnet\'s config file must be designated,\
             the configuration usually is src/lib/config/*.yaml file'
    return opt
